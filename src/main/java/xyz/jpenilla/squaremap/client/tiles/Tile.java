...
public class Tile {
    ...
    public void updateTexture() {
        synchronized (lock) {
            if (this.texture == null || this.texture.getPixels() == null) {
                SquaremapClientInitializer.instance().getScheduler().addTask(20, this::updateTexture);
                return;
            }
            for (int x = 0; x < SIZE; x++) {
                for (int z = 0; z < SIZE; z++) {
    ...
    public void render(PoseStack matrixStack, float x0, float y0, float x1, float y1, float u0, float v0, float u1, float v1) {
        updateLastUsed();
        SquaremapClientInitializer.instance().getTextureManager().drawTexture(matrixStack, this.identifier, x0, y0, x1, y1, u0, v0, u1, v1);
    }
